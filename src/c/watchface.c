...
static void window_load(Window *window) {
  ...
  watchface->time_layer = text_layer_create(GRect(0, 20, width, 58));
  watchface->date_layer = text_layer_create(GRect(0, 78, width, 22));
  watchface->heart_rate_layer = text_layer_create(GRect(0, height - 31, width, 24));
  watchface->heart_icon_layer = layer_create(GRect(width / 2 - 47, height - 28, 18, 18));
  ...
  text_layer_set_font(watchface->date_layer, fonts_get_system_font(FONT_KEY_GOTHIC_14_BOLD));
  ...
  cat_animation_init(&watchface->cat, root,
                     GRect((width - cat_width) / 2, cat_y, cat_width, cat_height),
